...
#pragma warning disable 0618
            ...
            var inclusiveFilterGuid = AssetDatabase.FindAssets("t: InclusiveTestFilters");
            var inclusiveFilters = AssetDatabase.LoadAssetAtPath<InclusiveTestFilters>(
                AssetDatabase.GUIDToAssetPath(inclusiveFilterGuid.FirstOrDefault()));

            if (inclusiveFilters != null && buildPlatform == inclusiveFilters.filter.BuildPlatform)
            {
                filterConfigs = null;
            }
            ...
            foreach (var scene in buildSettingsScenes)
            {
                ...
                {
                    scene.enabled = false;
                    if (inclusiveFilters.filter.FilteredScenes.Select(s => AssetDatabase.GetAssetPath(s)).Contains(scene.path))
                    {
                        scene.enabled = true;
                    }
                }
