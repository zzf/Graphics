...
class Template_AllTemplateCiJob():
    ...
    def __init__(self, templates, agent, platforms, editor):
        self.job_id = template_job_id_test_all(editor["name"])
    ...
    def get_job_definition(self, templates, agent, platforms, editor):
        ...
        for platform in platforms:
            for template in templates:
                dependencies.append(f'{templates_filepath()}#{template_job_id_test(template["id"],platform["os"],editor["name"])}')
        
        # construct job
        job = YMLJob()
        job.set_name(f'Pack and test all templates - { editor["name"] }')
        job.set_agent(agent)
